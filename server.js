...
const teamOf  = (role, teams) =>
  teams ? (teams.A.includes(role) ? 'A' : teams.B.includes(role) ? 'B' : null) : null;
...
function beginPlaying(state) {
  ...
  state.trumpCalled = false; // true once trump is led
...
function runPack(state) {
  ...
  if (!state.deck.length) {
    state.phase  = 'redeal';
    state.action = 'Pack depleted — reshuffling';
    return;
  }
...
function playCard(state, role, card) {
  ...
  const isTrump  = cardSuit === ts;
  const trickLen = Object.keys(state.trick).length;

  if (trickLen === 0) {
    // ── Leading a trick ──────────────────────────────────────────────
    // If you lead trump, mark trump as called
    if (isTrump) state.trumpCalled = true;
    ...
    if (leadTrump || state.trumpCalled) {
      // Trump was led (called) — everyone must follow trump if they have it
      const hasTrump = hand.some(c => suit(c) === ts);
      if (hasTrump && !isTrump) return 'Trump was called — you must play trump';
    } else {
      // Non-trump was led — normal follow-suit rules apply
      const hasLeadSuit = hand.some(c => suit(c) === leadSuit);
      if (hasLeadSuit) {
        // Must follow lead suit; trump may also be played freely
        if (!isTrump && cardSuit !== leadSuit)
          return 'Must follow suit (or play trump)';
      } else {
        // Void in lead suit — if playing trump, must beat the highest trump already in trick
        if (isTrump) {
          // Find highest trump already played this trick
          const trumpsInTrick = Object.values(state.trick)
            .filter(c => suit(c) === ts);
          if (trumpsInTrick.length > 0) {
            const highInTrick = Math.max(...trumpsInTrick.map(rankVal));
            const hasBetter   = hand.some(c => suit(c) === ts && rankVal(c) > highInTrick);
            if (hasBetter && rankVal(card) <= highInTrick)
              return 'You must overtrump (play a higher trump) if you can';
          }
        }
        // else: non-trump, void in lead suit — any non-trump card is fine
      }
    }
  }
...
function resolveTrick(state) {
  ...
  // Accumulate game points to the trick WINNER
  ...
  // Check if trump was called in this trick (winner led trump)
  // trumpCalled persists for the rest of the round once set
...
function scoreRound(state) {
  ...
  // GAME — sum game points per team using actual team membership
  const gpA = teams.A.reduce((acc, r) => acc + (state.gamePointsWon[r] || 0), 0);
  const gpB = teams.B.reduce((acc, r) => acc + (state.gamePointsWon[r] || 0), 0);
  if (gpA > gpB)      { pts.A += 2; det.A.game = gpA; }
  else if (gpB > gpA) { pts.B += 2; det.B.game = gpB; }
  ...
  if (state.scores.A >= 14)      { state.phase = 'game_over'; state.winner = 'A'; state.action = 'Team A wins! 🎉'; }
  else if (state.scores.B >= 14) { state.phase = 'game_over'; state.winner = 'B'; state.action = 'Team B wins! 🎉'; }
  else {
...
function makeState() {
  return {
    ...
    hands: { p0:[], p1:[], p2:[], p3:[] }, deck: [], turnUp: null,
    trumpSuit: null, trumpCalled: false,
...
wss.on('connection', ws => {
  ...
  ws.on('message', raw => {
    ...
    // JOIN
      ...
      r.state[myRole] = msg.name;
      r.state.playerCount = r.players.length + 1;
      r.players.push({ ws, role: myRole, name: msg.name });
      send(ws, { type:'joined', role: myRole, state: playerView(r.state, myRole) });
      broadcastAll(r, { type:'lobby_update', state: r.state });
    }

    // SET TEAMS (host only, lobby only)
    else if (msg.type === 'set_teams') {
      if (!room || myRole !== 'p0') return;
      const tA = msg.teamA, tB = msg.teamB;
      if (!tA || !tB || tA.length !== 2 || tB.length !== 2)
        return send(ws, { type:'error', msg:'Invalid teams — need 2 per team' });
      const allRoles = [...tA, ...tB].sort().join(',');
    ...
    // BEG
      ...
      s.phase  = 'give_one';
      s.action = `${pname(s, s.dealer)} — Give One or Run Pack?`;
      broadcastState(room);
    }
